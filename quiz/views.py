...
qna = [
    ...
    ('I have a fixed base but I must be raised to produce a given number.',
     'Merchiston Tower'),
...
def quiz_page(request):
    ...
    if request.method == "POST":
        ...
        if q_no == 10 or usr.attempt_for_q <= 0:
            return render(request, "quiz/thankyou.html")

        if q_no == 0 and usr.attempt_for_q==5:
        ...
        context = {}
        context['q_no'] = str(q_no+1)
        context['time_remaining'] = 1800+shown_at_time-int(datetime.timestamp(datetime.now()))
